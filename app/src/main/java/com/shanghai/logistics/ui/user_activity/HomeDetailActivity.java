...
public class HomeDetailActivity extends BaseActivity<UserSpecialPresenter> implements UserSpecialConnector.View {
    ...
    LStoreInfoEntity.DedicatedLine dedicatedLineList;
    ...
    public void UserSpecial(SpecialEntity e) {
        ...
        //时限
        mTvTime.setText(e.getAging() + "天直达");
        //商品介绍
    ...
    @Override
    public void LogisticsStoreInfo(LStoreInfoEntity e) {
        Log.i("UserSpecial", "UserSpecial: " + e.toString());
        //顶部标题
        Glide.with(this).load(Constants.MAIN_URL + e.getStoreName()).into(mImgHead);
        //专线名称
        mTvSpecialName.setText(e.getStoreName());
        //发货地和收货地
        mTvFormWhere.setText(e.getPosition());
        //所在地
        mTvCity.setText(e.getStoreAddress());
        //每吨价格
        mTvWight.setText(dedicatedLineList.getWeight() + "/吨");
        //每方价格
        mTvVolume.setText(dedicatedLineList.getVolume() + "方/吨");
        //月销
        mTvSquare.setText("月销" + dedicatedLineList.getTotalSales() + "笔");
        //时限
        mTvTime.setText(dedicatedLineList.getAging() + "小时直达");
        //商品介绍
        mTvIntroduction.setText(e.getIntroduction());

        dedicatedLinePhoneId = dedicatedLineList.getId() + "";

        // storeAccount=e.getPhone();

        dedicatedLineList = e.getDedicatedLine().get(0);
        Log.i(TAG, dedicatedLinePhoneId + "UserSpecial: " + dedicatedLinePhoneId + e.getIntroduction());
    }

    @Override
    public void LogisticsStoreInfoErr(String s) {

        Log.i(TAG, "LogisticsStoreInfoErr: "+s);
